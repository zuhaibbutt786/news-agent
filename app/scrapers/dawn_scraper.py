...
class DawnNewsScraper(BaseScraper):
    """
    Scraper for Dawn News (https://www.dawn.com/)
    """
    ...
    def fetch_articles(self, category=None, limit=10):
        ...
        try:
            ...
            # Find article links
            articles = soup.select('.story__link, .media__link, article a')
            ...
            logging.error(f"Error fetching articles from Dawn News: {str(e)}")
            return []
    
    def parse_article(self, url):
        """
        Parse a single article from Dawn News
        ...
        try:
            ...
            title_element = soup.select_one('.story__title, .article__title, h1')
            title = title_element.text.strip() if title_element else ""
            
            # Extract content
            content_elements = soup.select('.story__content p, .article__content p')
            content = '\n\n'.join([p.text.strip() for p in content_elements if p.text.strip()])
            
            # Extract date
            date_element = soup.select_one('.story__time, .article__time, time')
            ...
            if date_str:
                try:
                    ...
                    logging.error(f"Error parsing date from Dawn News: {str(e)}")
            ...
            # Extract category
            category_element = soup.select_one('.story__category, .article__category, .cat-name')
            category = category_element.text.strip() if category_element else "General"
            
            # Extract image URL
            image_element = soup.select_one('.story__image img, .article__image img, .media__item img')
            ...
            logging.error(f"Error parsing article from Dawn News: {str(e)}")
            return None